%%% Leverage limit constrain for:

 

% it saves down all the data to 'MPC_data.mat'

fprintf('*** Running MPCGARCH for the leverage limit constrain. ***\n')
fprintf('This takes about 15 minutes to run 3 iterations, depending on your machine.\n')
...
tic
for i=1:N1
    fprintf('optimizing for the period %i\n',i);
    ...
    for t=1:T 
        fprintf("Running for t= %i\n",t);
        %our code
        Var_Mat = zeros(T-t+1 ,30);
        for j=1:30
            mdl = gjr(3,3);
            EstMdl = estimate(mdl,Full_Data(1:K-3*T-1+t+(i-1)*100,j+1),'Display','off');
            Var_Mat(:,j) = forecast(EstMdl,T+1-t,Full_Data(1:K-3*T-1+t+(i-1)*100,j+1));
        end
        ...
        cs(t) = sum(u) + sh'*subplus(-x-u) + kappa'*abs(u) + s'*u.^2 + lambda*(x+u)'*Curr_Sigma*(x+u);
