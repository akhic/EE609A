...
% this script performs the optimization
% for the Model Predictive Control (MPC) policy

% it saves down all the data to 'MPC_data.mat'

fprintf('*** Running MPC for the leverage limit constrain. ***\n')
...
tic
for i=1:N1
    fprintf('optmizing for the period %i\n',i);
    ...
    for t=1:T 
        ...
        cs(t) = sum(u) + sh'*subplus(-x-u) + kappa'*abs(u) + s'*u.^2 + lambda*(x+u)'*sigma*(x+u);
        risk_penalty(t,i) = lambda*(x+u)'*sigma*(x+u);
        if(i==1)
