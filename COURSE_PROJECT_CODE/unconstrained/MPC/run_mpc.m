...
% this script performs the optimization over 3 time interval
% for the Model Predictive Control (MPC) policy

% it saves down all the data to 'MPC_data.mat'

fprintf('*** Running MPC for the unconstrained problem. ***\n')
...
for i=1:N1
    ...
    for t=1:T 
        ...
        cs(t) = sum(u) + sh'*subplus(-x-u) + kappa'*abs(u) + s'*u.^2 + lambda*(x+u)'*sigma*(x+u);% total cost at time t
